...
def loadData(spark, DBName, schema):
    ...
    for table, path in tables.items():
        try:
            ...
            logger.info(f"[LOADING DATA] Loading data Table : {table}")
            ...
            logger.info(f"[LOADING DATA] Success loaded data into Table : {table}")
        
        except Exception as e:
            logger.error(f"[LOADING DATA] [ERROR] Error when Data to Table in Database {DBName} : {e}")
            raise
    
    logger.info(f"[LOADING DATA] All data loaded successfully")